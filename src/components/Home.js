...
const renderCardTitle = (item, loggedIn, favs = [], favOnChange, category) => {
    ...
    const broadcasterUrl = `https://www.twitch.tv/${item.broadcaster_name}`;
    ...
    return (
        <>
            ...
            <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', width: 450 }}>
                <Tooltip title={title}>
                    {category === tabKeys.Streams ? (
                        <a href={broadcasterUrl} target="_blank" rel="noopener noreferrer">
                            <span>{title}</span>
                        </a>
                    ) : (
                        <span>{title}</span>
...
const renderCardGrid = (data, loggedIn, favs, favOnChange, category) => {
    return (
        <List
            grid={{
                xs: 1,
            ...
            renderItem={item => (
                <List.Item style={{ marginRight: '20px' }}>
                    <Card
                        title={renderCardTitle(item, loggedIn, favs, favOnChange, category)}
                    >
                        <a
                            href={item.url}
                            target="_blank"
    ...
    return (
        <Tabs defaultActiveKey={tabKeys.Streams}>
            <TabPane tab="Streams" key={tabKeys.Streams} forceRender={true}>
                {renderCardGrid(streams, loggedIn, favStreams, favoriteOnChange, tabKeys.Streams)}
            </TabPane>
            <TabPane tab="Videos" key={tabKeys.Videos} forceRender={true}>
                {renderCardGrid(videos, loggedIn, favVideos, favoriteOnChange, tabKeys.Videos)}
            </TabPane>
            <TabPane tab="Clips" key={tabKeys.Clips} forceRender={true}>
                {renderCardGrid(clips, loggedIn, favClips, favoriteOnChange, tabKeys.Clips)}
            </TabPane>
        </Tabs>
    );
};
